...
class KalasantyFeaturizer:
    ...
    def get_channels(self,mol):
        heavy_atoms_idxs, self.channels = self.featurizer.get_features(mol)  # returns only heavy atoms
        return heavy_atoms_idxs
    
    def get_channels_dummy(self,coords):
        features = np.zeros((len(coords),18))
        features[:,1] = 1
        features[:,9] = 1
