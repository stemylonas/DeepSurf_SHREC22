...
class Network:
    ...
    def get_lig_scores(self,prot,batch_size):
        heavy_atoms_idxs = self.featurizer.get_channels(prot.mol)
        heavy_atom_coords = prot.atom_coords[heavy_atoms_idxs]
        ...
        for p,n in zip(prot.surf_points,prot.surf_normals):
            input_data[batch_cnt,:,:,:,:] = self.featurizer.grid_feats(p,n,heavy_atom_coords)  
            batch_cnt += 1
